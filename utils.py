...
def translate_with_gemini_hebrew(text_content):
    try:
        ...
        2. Summarize into 3 bullet points if long.
        Input: {text_content}
        """
...
class AIBatchProcessor:
    ...
    async def analyze_batch(self, items):
        if not items: return []
        chunk_size = 5 # Increased for efficiency
        results = []
        system_instruction = """Output JSON: {"items": [{"id": 0, "title": "Hebrew Title", "summary": "Hebrew Summary"}]}"""
        
        for i in range(0, len(items), chunk_size):
            chunk = items[i:i+chunk_size]
            batch_text = "\n".join([f"ID:{idx} | Text:{x['title']} - {x['summary'][:1500]}" for idx, x in enumerate(chunk)])
            ...
            for j in range(len(chunk)):
                ...
                results.append({
                    "category": "News", "severity": final_sev, "title": heb_title,
                    "summary": heb_sum, "published_at": chunk[j]['date'], 
                    "actor_tag": chunk[j].get('actor_tag'), "tags": final_tag
                })
        return results

    async def analyze_single_ioc(self, ioc, ioc_type, data):
        lean_data = self._extract_key_intel(data)
        prompt = f"Act as Senior SOC Analyst. Target: {ioc} ({ioc_type}). Data: {json.dumps(lean_data)}. Output Hebrew Markdown."
        return await query_groq_api(self.key, prompt, json_mode=False)

    def _extract_key_intel(self, raw_data):
        summary = {}
        if raw_data.get('virustotal'):
...
class APTSheetCollector:
    def fetch_threats(self): 
        return [
            {"name": "MuddyWater", "origin": "Iran (MOIS)", "target": "Israel, Turkey, Jordan", "type": "Espionage", "tools": "PowerShell, Ligolo, ScreenConnect", "desc": "מזוהה עם משרד המודיעין האיראני."},
            {"name": "OilRig (APT34)", "origin": "Iran (IRGC)", "target": "Israel Critical Infra", "type": "Espionage", "tools": "Karkoff, SideTwist", "desc": "מתמקדת במגזרי פיננסים ואנרגיה."},
            {"name": "Agonizing Serpens", "origin": "Iran", "target": "Israel (Education, Tech)", "type": "Wiper", "tools": "BiBiWiper, Moneybird", "desc": "מטרתה השמדת מידע."}
        ]
            ...
            async with session.get(source['url'], headers=HEADERS, timeout=25) as resp:
                ...
                if source['type'] == 'rss':
                    ...
                    for entry in feed.entries[:10]:
                        items.append({"title": entry.title, "url": entry.link, "date": parse_flexible_date(getattr(entry, 'published_parsed', None)), "source": source['name'], "summary": BeautifulSoup(entry.summary, "html.parser").get_text()[:1000]})
