...
def init_db():
    ...
    # Cleanup: Keep INCD for 7 days (to ensure visibility), others for 48h
...
class AIBatchProcessor:
    ...
    async def analyze_batch(self, items):
        ...
        # PROMPT UPDATED: Distinct Title and Summary
        system_instruction = """
        ...
        try:
            ...
            async with session.get(source['url'], headers=HEADERS, timeout=25) as resp:
                ...
                if source['type'] == 'rss':
                    ...
                    # FALLBACK: If feed has items but date filter kills them, take the newest one anyway.
                    all_feed_items = []
                    ...
                    for entry, pub_date in all_feed_items:
                        ...
                        if (now - pub_date).total_seconds() > (limit_hours * 3600):
                            # Special INCD rule: If we have NO items yet, take this old one just to show something
                            if source['name'] == 'INCD' and len(items) == 0 and len(all_feed_items) > 0:
                                pass # Allow this item as a fallback
                            else:
                                continue
